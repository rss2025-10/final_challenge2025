...
class SafetyController(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter("module", "shrinkray_heist")
        ...
        if (self.MODULE == "shrinkray_heist"):
            self.scan_length = None
            self.max_speed = 1.5
            self.speed = 1.5
            self.HARD_STOP_BOUND = 0.7
            self.SLOW_BOUND = 0.8
        elif (self.MODULE == "race_to_the_moon"):
            self.scan_length = None
            self.max_speed = 1.5
            self.speed = 1.5        
            self.HARD_STOP_BOUND = 0.05
            self.SLOW_BOUND = 0.1
        else:
            raise ValueError("Invalid module name in safety controller!")
    ...
    def laser_scan_callback(self, msg):
        ...
        if not self.scan_length:
            ...
            if self.MODULE == "shrinkray_heist":
                ...
                self.left_bound = self.scan_length // 10
                self.right_bound = self.scan_length - self.scan_length // 10
        ...
        for k in use_range:
            ...
            if len(chunk_values) > 0:
                ...
                if average < self.HARD_STOP_BOUND:
